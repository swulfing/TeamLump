...
datasets <- c("MA_DMF_clean", "ME_DMR_clean", "NH_FG_clean", "NEFSC_Observer_clean", "NEFSC_BT_clean", "datasets")
...
MA_DMF_TOWCOUNT <- MA_DMF %>%
  group_by(LON, LAT, CRUISE ) %>%
  filter(!is.na(LON) & !is.na(LAT) & !is.na(CRUISE)) %>%
     filter(n()>1)


colSums(MA_DMF_TOWCOUNT != 0)
...
ME_DMR_TOWCOUNT <- Trawl_Tow %>%
  group_by(Survey, Tow_Number, Start_Latitude, Start_Longitude)
# %>%
#      filter(n()>1)
...
NHFG_TOWCOUNT <- NHFG_Catch %>%
  group_by(SampleID, StationName, Latitude, Longitude)
...
catch_data <- data.frame(read.csv("C:/Users/Sophi/Desktop/NEFSC_Observer_Data/NEFSC_Observer_CatchData.csv"))
length_data <- data.frame(read.csv("C:/Users/Sophi/Desktop/NEFSC_Observer_Data/NEFSC_Observer_LengthData.csv"))

NEFSC_Observer_TOWCOUNT <- catch_data %>%
  group_by(LINK3)
...
#Conversions
NEFSC_Observer_Combine <- catch_combine %>%
  rowwise() %>%
...
NEFSC_Observer_Combine$DATEHBEG <- as.POSIXct(NEFSC_Observer_Combine$DATEHBEG, format = "%m-%d-%Y %T")

NEFSC_Observer_clean <- NEFSC_Observer_Combine %>%
  select(DATEHBEG, LAT, LON, DEPTH, TEMP_SURFACE, WEIGHT_TOW, LENANML, WEIGHT_AVG, WEIGHT, NUMLEN, GEARNM) %>%
  rename(DATE = DATEHBEG,
         LENGTH = LENANML,
         NO_TOW = NUMLEN) %>% 
  mutate(DATA_SOURCE = "NEFSC_Observer", .before = DATE) %>%
...
# test <- NEFSC_Observer_clean %>%
#   filter_at(vars(LAT, LON, DATE), any_vars(is.na(.)))
# 
# Comparison <- NEFSC_Observer_clean %>%
...
```{r NEFSC_BT}
NEFSC_BT_sets <- c("22560", "22561", "22562", "22563")
seasons <- c("FALL", "SPRING", "SUMMER", "WINTER")
fileLocation <- "C:/Users/sophi/Desktop/NEFSC_BT_Data/"

NEFSC_BT_seasonlist <- list()

# NEFSC_BT Data separated by season collected. This for loop just goes through data from each season and applies same cleaning protocol
for(i in 1:length(NEFSC_BT_sets)){
# Note. Not using cruise dataset
environ_data <- data.frame(read.csv(paste0(fileLocation,"NEFSC_BT/", NEFSC_BT_sets[i], "_FSCSTables/", NEFSC_BT_sets[i], "_UNION_FSCS_SVSTA.csv")))
catch_data <- data.frame(read.csv(paste0(fileLocation,"NEFSC_BT/", NEFSC_BT_sets[i], "_FSCSTables/", NEFSC_BT_sets[i], "_UNION_FSCS_SVCAT.csv")))
bio_data <- data.frame(read.csv(paste0(fileLocation,"NEFSC_BT/", NEFSC_BT_sets[i], "_FSCSTables/", NEFSC_BT_sets[i], "_UNION_FSCS_SVBIO.csv")))
len_data <- data.frame(read.csv(paste0(fileLocation,"NEFSC_BT/", NEFSC_BT_sets[i], "_FSCSTables/", NEFSC_BT_sets[i], "_UNION_FSCS_SVLEN.csv")))
...
NEFSC_BT_season <- right_join(environ_data_clean, catch_data_clean,
                       by = c("SURVEY_ID", "SEASON")) %>%
  full_join(., indiv_data,
            by = "SURVEY_ID") %>%
  mutate(DATA_SOURCE = "NEFSC_BT")

NEFSC_BT_seasonlist[[i]] <- NEFSC_BT_season
} ## End for loop, still need to stack all datasets

#write.csv(NEFSC_BT_clean, "C:/Users/sophi/Desktop/NEFSC_BT_Data/NEFSC_BT_GearTest.csv")

NEFSC_BT_clean <- Reduce(full_join, NEFSC_BT_seasonlist)
rm(list=setdiff(ls(), datasets))
```

```{r combine}

Lumpfish_Data_clean <- plyr::rbind.fill(MA_DMF_clean, ME_DMR_clean, NH_FG_clean, NEFSC_Observer_clean, NEFSC_BT_clean)
...
NOAAObserver_gearCount_ALL <- Lumpfish_Data_clean %>%
  filter(DATA_SOURCE == "NEFSC_Observer") %>%
  group_by(GEARNM) %>%
  summarize(count = n()) %>%
  arrange(desc(count))
  

write.csv(NOAAObserver_gearCount_ALL, "NOAAObserver_gearCount_ALL.csv", row.names = FALSE)

NOAAObserver_gearCount_2021 <- Lumpfish_Data_clean %>%
  filter(DATA_SOURCE == "NEFSC_Observer") %>%
  filter(YEAR <= 2021) %>%
  group_by(GEARNM) %>%
  summarize(count = n()) %>%
...
write.csv(NOAAObserver_gearCount_2021, "NOAAObserver_gearCount_2021.csv", row.names = FALSE)
